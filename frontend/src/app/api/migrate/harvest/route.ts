...
import { buildHarvestPlan } from "@/services/server";
...
export async function POST(request: Request) {
  ...
  try {
    const plan = await buildHarvestPlan(owner, tokenId, request.signal, {
      riskProfile,
    ...
    const message =
      err instanceof Error ? err.message : "Failed to build harvest plan";
